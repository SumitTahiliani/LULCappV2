...
import io
import tempfile
from rasterManager import get_or_generate_rf_raster
import json
...
def downsample_and_mask(src, geojson_geom, scale_factor=0.2):
    try:
        geom_in_raster_crs = [transform_geom("EPSG:4326", src.crs.to_string(), g) for g in geojson_geom]
        out_image, _ = mask(src, geom_in_raster_crs, crop=True)
...
@st.cache_data
@st.cache_data
def analyze_aoi(location, use_model=True):
    from rasterManager import get_or_generate_rf_raster

    records = {}
    images_by_year = {}
    from shapely.geometry import shape
    polygon = shape(location[0]['geojson'])
    bbox = polygon.bounds  # (minx, miny, maxx, maxy)
    geojson_geom = [mapping(polygon)]
    for year in range(2024, 2026):
        try:
            if use_model:
                path = get_or_generate_rf_raster(location, year)
            else:
                path = get_raster_path(year)
                if not path or not os.path.exists(path):
                    continue
        except Exception as e:
            st.warning(f"Could not load raster for {year}: {e}")
            continue
        ...
        try:
            ...
            st.warning(f"Failed to read raster for {year}: {e}")

    if not records:
        st.warning("No valid raster data found for this AOI and year range.")
        return pd.DataFrame(), {}
...
use_model = st.checkbox(
    "Use Random Forest Model (instead of static rasters)",
    value=True
)
    ...
    for idx, loc in enumerate(search_location(query)):
        ...
        with st.expander(name):
            if st.button(f"Analyze {name}", key=f"btn-{idx}"):
                ...
                st.session_state["selected_location"] = [loc]  # Save full location!
...
if "selected_bbox" in st.session_state:
    ...
    with st.spinner("Running analysis..."):
        df, images = analyze_aoi(st.session_state["selected_location"], use_model=use_model)
        # df, images = analyze_aoi(location, use_model=use_model)
        if not df.empty:
            ...
            if st.session_state["show_overlay"]:
                try:
                    tif_path = get_or_generate_rf_raster(st.session_state["selected_location"], 2025)
                    with rasterio.open(tif_path) as src:
                        from rasterio.warp import transform_geom
                        aoi = [transform_geom("EPSG:4326", src.crs.to_string(), mapping(box(*buffered)))]
